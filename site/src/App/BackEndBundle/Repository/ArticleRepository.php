...
class ArticleRepository extends \Doctrine\ORM\EntityRepository
{
        ...
        if ($query) {
            ...
            if (count($words) > 1) {
                foreach ($words as $key => $word) {
                    $queryString = array();
                    $queryString[] = "CONCAT(COALESCE(e.title,''),COALESCE(e.description,'')) LIKE :word" . $key;
                    $qb->setParameter('word' . $key, "%" . $word . "%");
                    $qb->andWhere(join(' AND ', $queryString));
                }
            } else {
                $qb->andWhere("CONCAT(COALESCE(e.title,''),COALESCE(e.description,'')) LIKE :query")->setParameter('query', "%" . $query . "%");
            }
        }
        return $qb;
    }
    ...
    public function getTotalByDay()
    {
        $qb = $this->createQueryBuilder('o')
            ->select('DATE_FORMAT(o.createdAt, \'%d/%m/%Y\') AS name, COUNT(o.id) AS total')
            ->groupBy('name');
